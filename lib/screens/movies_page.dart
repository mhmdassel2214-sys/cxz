...
class MoviesPage extends StatefulWidget {
  const MoviesPage({super.key, this.onAttachScrollToTop});
...
class _MoviesPageState extends State<MoviesPage> {
  ...
  void initState() {
    ...
    _connectionSubscription = ConnectivitySignal.onlineStream.stream.listen((isOnline) {
      if (!mounted || !isOnline) return;
      setState(() => moviesFuture = ApiService.fetchMovies());
    });
  }
  ...
  Future<void> _saveOffline(MovieItem movie) async {
    ...
    final message = saved.isDownloading
        ? 'بدأ تنزيل الفيلم وسيظهر تقدّم التحميل في الأوفلاين'
        : saved.isDownloaded
            ? 'الفيلم موجود مسبقًا في الأوفلاين'
            : saved.isStreamOnly
                ? 'هذا الرابط بث فقط ويمكن مشاهدته أونلاين'
                : 'تمت إضافة الفيلم إلى الأوفلاين';
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(message)),
    );
  }

  int _gridCrossAxisCount(double width) => width >= 900 ? 4 : width >= 650 ? 3 : 2;

  double _wideCardWidth(double width) => width >= 900 ? 250 : width >= 650 ? 220 : 180;

  @override
  Widget build(BuildContext context) {
    final width = MediaQuery.of(context).size.width;
    final horizontalPadding = width < 360 ? 12.0 : 14.0;
    final searchHeight = width < 360 ? 48.0 : 52.0;
    final recentHeight = width >= 650 ? 280.0 : 250.0;
              ...
              child: Container(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    contentPadding: EdgeInsets.symmetric(horizontal: width < 360 ? 12 : 14, vertical: 14),
                  ),
                ),
              ),
            ),
                            ...
                            children: [
                              ...
                              GridView.builder(
                                ...
                                gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                                  ...
                                  childAspectRatio: width >= 650 ? 0.68 : 0.60,
                                  mainAxisSpacing: 14,
                                  crossAxisSpacing: 14,
                                ),
                                itemBuilder: (_, i) {
...
class _WideMovieCard extends StatelessWidget {
  ...
  final double width;
  const _WideMovieCard({required this.movie, required this.showNewBadge, required this.onTap, required this.width});
